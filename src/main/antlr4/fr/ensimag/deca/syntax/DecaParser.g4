...
ListInst list = null;
    ...
    : if1=IF OPARENT condition=expr CPARENT OBRACE li_if=list_inst CBRACE {
            ListInst main_if = new ListInst();
            $tree = new IfThenElse($condition.tree, $li_if.tree, main_if);
            setLocation($tree, $if1);
            list=main_if;
        }
      (ELSE elsif=IF OPARENT elsif_cond=expr CPARENT OBRACE elsif_li=list_inst CBRACE {
            ListInst clauseElse = new ListInst();
            IfThenElse clauseIF = new IfThenElse($elsif_cond.tree, $elsif_li.tree, clauseElse);
            setLocation(clauseIF, $elsif);
            //we add the ifclause in the list
            list.add(clauseIF);
            list = clauseElse;
        }
      )*
...
mult_expr returns[AbstractExpr tree]
    ...
    | e1=mult_expr TIMES e2=unary_expr {
            assert($e1.tree != null);                                         
            assert($e2.tree != null);
            $tree = new Multiply($e1.tree, $e2.tree); // a implementer apres
            setLocation($tree, $TIMES);
        }
    | e1=mult_expr SLASH e2=unary_expr {
            assert($e1.tree != null);                                         
            assert($e2.tree != null);
            $tree = new Divide($e1.tree, $e2.tree); // a implementer apres
            setLocation($tree, $SLASH);
        }
    | e1=mult_expr PERCENT e2=unary_expr {
            assert($e1.tree != null);                                                                          
            assert($e2.tree != null);
           $tree = new Modulo($e1.tree, $e2.tree); // a implementer apres
...
type returns[AbstractIdentifier tree]
    : ident { $tree = $ident.tree; }
        | t=(TYPE_INT | TYPE_FLOAT | VOID | TYPE_BOOLEAN) {
